...
func circuitBreakerStop(ctx context.Context, policy StopPolicy, cbC *cbContainer, request interface{}, cbKey string) (bool, string) {
	...
	if errRate < policy.CBPolicy.ErrorRate {
		klog.Infof("retry circuit break closed[%s], errRate=%0.9f, sample=%d", cbKey, errRate, sample)
		return false, ""
	}
	klog.Infof("retry circuit break[%s], errRate=%0.9f, sample=%d", cbKey, errRate, sample)
